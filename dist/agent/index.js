...
import { ANTHROPIC_API_KEY_ENV_KEY, BASETEN_API_KEY_ENV_KEY, FIREWORKS_API_KEY_ENV_KEY, getDefaultModelId, getProviderApiKeyEnvKey, getProviderConfig, getProviderLabel, isValidModelId, normalizeModelId, OPENAI_API_KEY_ENV_KEY, OPENROUTER_API_KEY_ENV_KEY, OPENROUTER_BASE_URL, OPENROUTER_FALLBACK_MODEL_IDS, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_PROVIDER_ENV_KEY, resolveConfiguredProvider, } from "../constants.js";
import { createOpenWikiContentSnapshot, createRunContext, writeLastUpdateMetadata, } from "./utils.js";
export async function runOpenWikiAgent(command, cwd = process.cwd(), options = {}) {
    ...
    if (providerConfig.baseURL) {
        emitDebug(options, `provider.baseUrl=${JSON.stringify(providerConfig.baseURL)}`);
    }
...
function ensureProviderKey(provider) {
    ...
    if (!process.env[apiKeyEnvKey]) {
        throw new Error(`${apiKeyEnvKey} is required to run OpenWiki with ${getProviderLabel(provider)}.`);
    }
}
...
async function createModel(provider, modelId) {
...
function repairOllamaToolCallArguments(body) {
    let payload;
    try {
    ...
    for (const choice of payload.choices) {
        ...
        const toolCalls = isRecord(message) && Array.isArray(message.tool_calls)
            ? message.tool_calls
            : [];
...
function unwrapStringifiedJsonValues(argumentsJson) {
    let args;
    try {
